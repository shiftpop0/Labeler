...
2026-09-16 从同一黄金项目适配并同步以下通用交互改进，未复制业务数据或黄金类别定义：

- `W` 按配置顺序循环切换对象类别，保留“未选中框时设置下一框、明确选中时修改该框”的语义；
- `E` 触发既有“保存并下一张”流程，`Q` 继续切换新增框模式；
- Q/W/E 对输入区、组合键及长按重复事件不响应；
- 新框拖拽期间隐藏类别文字，鼠标松开完成框后再显示，已有框调整时仍显示类别。
